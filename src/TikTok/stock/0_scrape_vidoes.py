...
ACCOUNT_NAME = "GlobalTradePilot"   #"GlobalTradePilot"、 "StockPulseTrader"
...
MESSAGES = [
    ...
    "Nice comment on that golf video. I actually work with these setups, want to chat?"
]
MESSAGES = [
...
async def main():
    ...
    async with async_playwright() as p:
        ...
        for v_idx, video in enumerate(unique_videos):
            ...
            print(f"\n({v_idx+1}/{len(unique_videos)})")
            print(f"📊 正在爬取视频评论: {v_url}")
            ...
            try:
                ...
                for c in comments:
                    ...
                    if v_title.strip() == "" or c['text'].strip() == "": continue

                    # AI 意图判定 (参考 scrape_reviews_v0 逻辑)
                    prompt = "【角色】\n"\
                            "你是一位拥有 10 年经验的金融投资教育领域市场分析师，擅长通过社交媒体的碎片化信息捕捉用户对投资交易课程的购买信号（Buying Signals）\n\n"\
                            "【任务】\n"\
                            f"我将为你提供帖子的【正文内容】和用户的【评论内容】。请你分析该用户是否在咨询投资或购买股票的意见或者有购买股票/投资交易相关课程的潜在意图。\n\n"\
                            "【判定维度】\n"\
        ...
        for idx, lead in enumerate(all_potential_leads):
            ...
            if USE_PERSONALIZED_MESSAGE:
                prompt = "【角色】\n"\
                        ...
                        "\n"\
                        f"任务：我将为你提供【帖子内容】和【评论内容】。请根据这两个信息，生成一段个性化的英文私信。\n"\
                        "\n"\
                        "【写作准则】\n"\
                        "\n"\
                        "开场白（必选，结构分两步）：\n"\
            ...
            print(f'📚 原贴内容:{lead["Source_Title"]}\n')
            print(f'📝 评论内容:{lead["Comment"]}\n')
            print(f'📝 文案内容: {message}\n\n')
            
            potential_customer_data.append([
                target_id, lead['Source_Video'], lead['Source_Title'], lead['Comment'], message
            ])
            ...
            try:
                ...
                success = await send_direct_message(target_id, message)
        ...
        with open(TARGET_USERS_FILE, 'w', newline='', encoding='utf-8') as file:
            potential_customer_data.insert(0, ['uid', 'source url', 'source title', 'source comment', 'message'])
